...
export async function loader({ request }: { request: Request }) {
  ...
  if (token) {
    ...
    if (row) {
      db.prepare(
        "UPDATE users SET email_verified = 1, ever_verified = 1, verification_token = NULL, verification_expires = NULL WHERE id = ?"
      ).run(row.id);
      return redirect("/login?verified=1");
    }
    return { sent: !!sent, hasUserId: !!userId, invalidToken: true };
  }

  return { sent: !!sent, hasUserId: !!userId, invalidToken: false };
}

export default function VerifyEmail() {
  const { sent, invalidToken } = useLoaderData<typeof loader>();
    ...
    <main className="min-h-screen flex flex-col items-center justify-center bg-[#f5f5f7] dark:bg-[#1c1c1e] p-6">
      <div className="w-full max-w-md rounded-3xl bg-white dark:bg-neutral-800/80 p-8 shadow-sm dark:shadow-none border border-neutral-200/60 dark:border-neutral-700/60 text-center space-y-5">
        <h1 className="text-[22px] font-semibold text-neutral-900 dark:text-white">
          {invalidToken ? "Link expired or invalid" : "Check your email"}
        </h1>
        {invalidToken ? (
          <p className="text-[15px] text-neutral-500 dark:text-neutral-400 leading-relaxed">
            This verification link is no longer valid — it may have expired or already been used. Sign in, or request a new link from your account settings.
          </p>
        ) : sent ? (
          <p className="text-[15px] text-neutral-500 dark:text-neutral-400 leading-relaxed">
